  ...
  if (placed) {
    return (
      <View style={{ flex: 1, backgroundColor: t.surface, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 30 }}>
        ...
        <Txt style={{ fontWeight: '800', fontSize: 28, color: t.text, marginBottom: 10 }}>Order placed!</Txt>
        <Txt style={{ fontSize: 15, color: t.muted, lineHeight: 22, textAlign: 'center', marginBottom: 32 }}>
          {placed.count} item{placed.count === 1 ? '' : 's'} arriving via <Txt style={{ color: t.text, fontWeight: '700' }}>{placed.store.name}</Txt> · {placed.store.eta}. They’re checked off your grocery list.
        </Txt>
        <View style={{ width: '100%', gap: 12 }}>
          <PrimaryButton t={t} full onPress={() => router.replace('/(tabs)/grocery')}>View grocery list</PrimaryButton>
          <PrimaryButton t={t} ghost full onPress={() => router.replace('/(tabs)')}>Back to home</PrimaryButton>
        </View>
      </View>
    );
  }
  ...
  return (
    <Screen>
      ...
      <View style={{ gap: 12 }}>
        ...
        {toBuy.length === 0 ? (
          <Txt style={{ textAlign: 'center', color: t.muted, paddingVertical: 18 }}>Everything’s already checked off — nothing to order.</Txt>
        ) : null}
      </View>
    </Screen>
  );
